$(document).ready(function(){
    $.getJSON('../public/json/cards.json',function(data){
        ...
        data.forEach(function(section){
            contentHtml+=`<section><h2>`+section.category+`</h2><div class="scroll-section">`+
            section.items.map(function(item){
                return '<div class="card"><img src="'+item.image+'" alt="'+item.title+'"><h3>'+item.title+'</h3><p>'+item.description+'</p></div>';
            }).join('')+'</div></section>';
        });
        $('#card-output').html(contentHtml);initCardEvents();
    });

    function initCardEvents(){
        $('body').append('<div class="card-overlay"></div>');

        $('.card').each(function(){ $(this).data('originalContent',$(this).html()); });

        $('.card').click(function(){ var $this=$(this); if($this.hasClass('card-expanded'))return;
            $('.card-overlay').fadeIn(300);
            var expandedContent=$this.data('originalContent')+`<p style="margin-top:15px;"><button class="btn">Contact For Support</button></p><span class="card-close">&times;</span>`;
            $this.html(expandedContent).addClass('card-expanded'); 
        });

        $(document).on('click','.card-close',function(e){ e.stopPropagation(); var $card=$(this).closest('.card'); $card.html($card.data('originalContent')).removeClass('card-expanded'); $('.card-overlay').fadeOut(600); });
